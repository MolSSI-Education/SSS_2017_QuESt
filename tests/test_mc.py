...
def test_sys_ene():
    energy = quest.mc_demo.system_energy(coordinates[:,0], coordinates[:,1], coordinates[:,2], 10.0, 9.0)
...
def test_pair_ene():
    ...
    for i_particle in range(0, num_particles):
        molenergy = quest.mc_demo.pair_energy(i_particle, coordinates[:,0],coordinates[:,1], coordinates[:,2], 10.0 , 9.0)
        tot += molenergy
    assert np.round(tot/2)==np.round(-4351.540194543863)
    return
def test_rdf():
    (r, rdf, gr_max, r_max)=quest.rdf_func(delta_r, gr, coordinates, num_particles, box_length, cutoff2, gr_ideal, r_domain, number_of_snapshots) 
