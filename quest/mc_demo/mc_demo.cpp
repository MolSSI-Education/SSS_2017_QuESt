...

{
  ...
  if (x_coords_info.ndim != 1)
      throw std :: runtime_error("x_coords is not an one dimensional array");
  if (y_coords_info.ndim != 1)
      throw std :: runtime_error("y_coords is not an one dimensional array");
  if (z_coords_info.ndim != 1)
      throw std :: runtime_error("z_coords is not an one dimensional array");
...
#pragma omp parallel for schedule(static) reduction(+:e_pair)
  for (int k = 0; k < num_particles; k++)
  {
         if (par_num != k)
         {
           double j_x = x_data[3*k];
           double j_y = y_data[3*k];
           double j_z = z_data[3*k];
...
py::array_t <double> rdf(double delta_r, 
                                    py::array_t <double> gr,
